...
## e7c50f9 — 2026-09-11 (تفريغ البيانات التجريبية)
**ما تغيّر:** القاعدة صارت فارغة وجاهزة للبيانات الحقيقية: فهرس الأثمان (477) + حساب مدير النظام admin@mutqin.ly فقط — لا مراكز ولا محفّظين ولا طلاب ولا أولياء أمور ولا سجلات، وعدّادات الأكواد من صفر (أول مركز C1، أول محفّظ T1، أول طالب S1، أول ولي أمر P1). يغني هذا التفريغ عن تفريغ 2026-09-10 وعن SQL الهجرتين (09-08 و09-11) لأنه يشملهما.
**الملفات:**
  - backend/database/seeders/ProductionSeeder.php  (added — لا يعمل على الخادم؛ لإعادة الإنتاج محلياً)
  - C:\mutqin-deploy\mutqin-clean-2026-09-11.sql  (التفريغ الكامل — بلا توكنات)
**يحتاج رفع؟** yes (قاعدة البيانات فقط)
**طريقة الرفع:** sql — phpMyAdmin على mutqinly_mutqin: استيراد mutqin-clean-2026-09-11.sql مباشرة (يحوي DROP TABLE IF EXISTS لكل جدول). كلمة مرور الأدمن الأولية سُلِّمت لصاحب المشروع في المحادثة — تُغيَّر من «ملفّي الشخصي» بعد أول دخول. يُنهي كل الجلسات الحالية.
**حالة الرفع:** pending

## c80e8f5 — 2026-09-10
**ما تغيّر:** بيانات الديمو صارت مركزاً واحداً (بلال بن رباح) و50 طالباً بدل 4 مراكز و130 — 5 محفّظين، مدير نشط + معطَّل، 26 ولي أمر، 3 طلبات إضافة قديمة بلا طلبات نقل. كلمة المرور الموحّدة mutqin2027.
**الملفات:**
  - backend/database/seeders/LibyanDataSeeder.php  (modified — لا يعمل على الخادم؛ للتوثيق)
  - C:\mutqin-deploy\mutqin-demo-2026-09-10-clean.sql  (تفريغ كامل للقاعدة بعد migrate:fresh --seed — بلا توكنات)
**يحتاج رفع؟** yes (قاعدة البيانات فقط — لا ملفات كود تخص الخادم)
**طريقة الرفع:** sql — في phpMyAdmin على mutqinly_mutqin: إسقاط كل الجداول ثم استيراد mutqin-demo-2026-09-10-clean.sql (التفريغ يحوي DROP TABLE IF EXISTS لكل جدول فيمكن استيراده مباشرة). يُنهي كل جلسات الدخول الحالية.
**حالة الرفع:** pending
