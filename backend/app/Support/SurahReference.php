...
class SurahReference
{
    ...
    /**
     * الأسماء الشائعة للأجزاء (بكلماتها الافتتاحية المتعارف عليها) — للبحث فقط.
     * جزء قد يحمل أكثر من اسم متداول (27: «قال فما خطبكم» ويشيع «الذاريات»
     * لأنه يبدأ داخلها). المطابقة عبر juzFromName() بالتطبيع الموحّد.
     */
    ...
    /**
     * رقم الجزء من اسمه الشائع — أو null إن لم يطابق.
     * التطبيع بالمصدر الموحّد ArabicText («عم» = «عمّ» = «عمَّ»)، مع تجاهل
     * سابقة «جزء/الجزء» («جزء عم» تعمل). الطرفان يُطبَّعان بنفس الدالة
     * (normalizeQuery) حتى تتطابق الأسماء المبدوءة بواو («والمحصنات» = «المحصنات»).
     */
    public static function juzFromName(string $name): ?int
    {
        static $map = null;
        if ($map === null) {
            $map = [];
            foreach (self::JUZ_NAMES as $juz => $aliases) {
                foreach ($aliases as $alias) {
                    $map[ArabicText::normalizeQuery($alias)] = $juz;
                }
            }
        }

        $q = ArabicText::normalizeQuery($name);
        $q = trim((string) preg_replace('/^جزء\s*/u', '', $q)); // «جزء عم» / «الجزء عم»
