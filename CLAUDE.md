...
- **Activate/deactivate replaces deletion** (approved decision): teachers, centers, students, and center managers are never hard-deleted — each has a `PUT .../{id}/status` toggle with audit fields (`status_changed_by/at`). Deactivating a user **revokes all their tokens immediately** (same S1 mechanism as password change); deactivating a center revokes the tokens of all its members and blocks their login; deactivating a manager leaves the requests pending for their center **pending** (they never devolve to the admin — the admin only gets an informational `manager_deactivated` notification linking to the managers page). Inactive students stay in history but drop out of attendance, fingerprint import, and reports (`is_active` filters); student lists default to `status=active` (`?status=inactive|all` to override).
...
- **Center manager panel** (`CenterManagerController` + `manager` gate): a per-center operations role. Everything is scoped to the manager's own `center_id` **taken from the account, never from the request** (client-sent `center_id` is overwritten/ignored). Can: view a center dashboard, list/add/edit (not delete) teachers of their center (`role` and `center_id` forced server-side), add/toggle students, import + review + correct fingerprint attendance (`corrected_by/at` audit, out-of-center record → 403), pull center-scoped report groups (`reportsSystem`/`reportsManagement` reusing `ReportService`) and manager PDF variants, and own the **student-request** flow for their center: approve/reject every request **targeting their center** (adds from their teachers, transfers from other centers — `StudentRequestController::assertManagerScope`), and create transfer requests from their center to another center (`POST /manager/student-requests`, target picked from `GET /manager/centers`).
...
- **Student requests** (`StudentRequestController`, `student_requests`): the **center manager is the only authority** — the system `admin` is never a party (no admin routes, no admin notifications, no admin page). Teachers have **no** request routes or page either (the old teacher add/transfer flow and `/student-requests` were removed from the base). Flows: (1) **legacy add rows** (`type=add`, created before the removal) stay approvable/rejectable by the manager of their target center until drained (approval creates the student under the requesting teacher). (2) **transfer** — the manager of the **source** center A creates a request for one of A's active students to another active center B (`target_teacher_id` stays null) → only the **active manager of B** is notified and can approve (optionally assigning a B teacher in the approve body; otherwise the student lands with `teacher_id = null` and `former_teacher_name` set) or reject. `managerIndex` returns `direction: incoming|outgoing` per row.
...
- **In-app notifications**: Laravel database channel via the generic `App\Notifications\InAppNotification` (`sendSafe()` never breaks the primary operation). Types: `request_created/approved/rejected` (target-center manager / requester — links are `manager/requests.html` for managers and `teacher/students.html` for teachers, never an admin page), `manager_deactivated` (admin, informational), `memorization_added`, `test_added` (parents). Bell UI in `layout.js` polls every 60s.
...
| GET | `/manager/centers` | manager | `CenterManagerController@otherCenters` (active centers except own — transfer targets) |
| GET/POST | `/manager/student-requests` (+`/{id}/approve`, `/{id}/reject`) | manager | `StudentRequestController` (`managerIndex`/`managerStore`; approve/reject only requests targeting own center) |
...
- **ManagerManagementController** — admin-only CRUD (no destroy) + `toggleStatus` for center managers; one manager per center; fixed email scheme `{latin}.centeradmin@mutqin.ly`; deactivation leaves the center's pending requests pending and sends admins an informational `manager_deactivated` notification (link `admin/managers.html`).
...
- **StudentRequestController** — manager-only: transfer requests (`managerStore`, source = own center, target must be another active center with an active manager); `managerIndex` (incoming + outgoing of own center); approve/reject gated by `assertManagerScope` (target-center manager only; admin/teacher → 403); approval re-validates (duplicate national id, chosen/target teacher belongs to the target center) and executes inside a transaction; notifications go to the target-center manager (create) and the requester (approve/reject).
...
- **admin/**: `dashboard`, `centers`, `teachers`, `students`, `managers`, `reports` (no requests page — the admin is not a party to student requests; there are no admin request routes either).
- **manager/**: `dashboard`, `students`, `teachers`, `attendance`, `attendance-review`, `requests`, `reports`.
- **teacher/**: `dashboard`, `students`, `attendance`, `memorization`, `weekly-tests`, `profile`, `reports` (no requests page and no request endpoints — teachers are not a party to student requests at all).
